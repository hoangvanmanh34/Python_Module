...
class Image_Alz():
    ...
    def Show_Result(self, lineS, dev, index):
        try:
            ...
            print(lineS)
            if index == 2:
                cv2.putText(self.image, str(dev[1]), (lineS[0][4], lineS[0][5]), cv2.FONT_HERSHEY_SIMPLEX, 2, color, 2,
                            cv2.LINE_AA)
                ...
                cv2.putText(self.image, str(dev[1]),
                            (round((lineS[-1][4] + lineS[0][3] + lineS[0][4]) / 2), lineS[0][5]),
    ...
    def Save_Log(self, dev1, dev2, dev3):
        ...
        sum_file.write(
            self.MAC + '\t' + str(self.TopFront).upper() + '\t' + self.dev_result + '\t' + timetosave + '\t' + str(
                dev1) + '\t' + str(dev2) + '\t' + str(dev3) + '\n')
        sum_file.close()
    ...
    def Offset(self, lineS, golden_test):
        try:
            ...
            if self._Offset > 22:  # and not golden_test:
                self.dev_result = 'FAIL'
