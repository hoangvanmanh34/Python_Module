...
class App(Frame):
    def __init__(self, master=None):
        ...
        self.lbStatus = tk.Label(self.Frame1, text='STANDBY', borderwidth=4, font='serif 25',
                                 width=20, height=2,
                                 bg='white', fg='blue')
        self.lbStatus.grid(row=0, column=1, sticky=W, padx=5, pady=5)
        # -------------------------------------------
        ...
        self.log.grid(row=1, column=0, columnspan=2, sticky=W, pady=5)
    ...
    def Check_DUT_Alive(self):
        while True:
            try:
                ...
                #self.Test_Flag = False
                alive = False
                if not self.bViewScreen:
                    alive = self.iTake.Check_DUT_Alive()
                #print('alive:'+str(alive))
                if alive and not self._isGoldenTest:
                    if not self.isRun:
                        #self.Test_Flag = True
                        #self.countTime = 0
                    ...
                    if self.isRun and not self._isGoldenTest:
                        self.isRun = False
                        self.Test_Flag = False
        ...
        try:
            ...
            if not result[0]:
                ...
                if tk.messagebox.askquestion('Manual Test',
                    ...
                    self.log.insert(END, '-->CHECK CAMERA TOP\r\n')
                    self.log.insert(END, '-->Parallela spec: [-1:1]\r\n')
                    self.log.insert(END, '-->OffSide spec: [-100:100]\r\n')
                    self.bfront = False
