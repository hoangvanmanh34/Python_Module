...
class DannyViewer(ControlMixin):
    ...
    def deploy_server(self, max_width=1024, bitrate=8000000, max_fps=0):
        #try:
            ...
            subprocess.Popen('D:/CameraVI/adb/adb shell CLASSPATH=/sdcard/scrcpy-server.jar app_process / com.genymobile.scrcpy.Server 1.12.1 800 8000000 30 true - false true')
            sleep(1)

            logger.info("Forward server port...")
            #subprocess.Popen(['D:/CameraVI/adb/', 'forward', 'tcp:8081', 'localabstract:scrcpy']).wait()
            subprocess.Popen('D:/CameraVI/adb/adb forward tcp:8081 localabstract:scrcpy').wait()
            sleep(2)
